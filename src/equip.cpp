...
{
    auto const slot = find_slot_(es);
    BK_PRECONDITION(slot);
    return *slot;
}
...
bkrl::equipment::equip(item & i)
{
    ...
    auto const slots = i.slots();
    auto required_slots = slots;
    ...
    return make_result(result_t::status_t::ok, slots);
}
